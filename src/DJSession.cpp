...
DJSession::DJSession(const std::string &name, bool play_all)
    : session_name(name),
      library_service(),
      controller_service(),
      mixing_service(), // שונה מהעדכון שלהם
      config_manager(),
      session_config(),
      track_titles(),
      play_all(play_all),
...
bool DJSession::load_track_to_mixer_deck(const std::string &track_title)
{
    ...
    int mix_result = mixing_service.loadTrackToDeck(*cached_track);
    if (mix_result == 0)
    {
        stats.deck_loads_a++;
        stats.transitions++;
    ...
    while (continue_session){
        ...
        else
        {
            ...
            if (selected_playlist.empty())
                return;
            while (!load_playlist(selected_playlist))
            {
